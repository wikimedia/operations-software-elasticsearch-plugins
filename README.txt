...
If debian/blobs looks good the developper can git commit -a && git review
(NOTE that debian/blobs is never uploaded to gerrit)

II. REVIEW A PATCH
-----------------
The process relies on the fact that at least two enginners will fetch the
...
If everything looks good they can scp the whole dir to copper and then build
the package with
...
It then verified 3 times:
 ...
 3. On copper by dpkg-buildpackage

V. FILES
---------
debian/rules            Makfile for dh and custom build steps
